...
import { createTaskTools } from "./task-tools.service.js";
...
export async function streamAgentReply(input: StreamAgentReplyInput) {
  ...
  if (!process.env.OPENAI_API_KEY && !process.env.GOOGLE_GEMINI_API_KEY) {
    throw new Error("Either OPENAI_API_KEY or GOOGLE_GEMINI_API_KEY must be set in env");
  }
  ...
  const agent = new Agent({
    ...
    model: modelName(),
    tools: {
      ...createCalendarTools(input.authUserId),
      ...createTaskTools(input.authUserId),
    },
    memory,
  });
